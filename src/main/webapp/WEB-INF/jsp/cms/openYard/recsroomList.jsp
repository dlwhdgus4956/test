...
<body>
	                    ...
	                    <div class="search_opt">
	                        <div class="search">
	                            ...
	                            <select name="searchtype" id="searchtype">
	                                ...
	                                <option value="1" <c:if test="${params.searchtype == '1' }">selected="selected"</c:if>>작성자</option>
	                                <option value="2" <c:if test="${params.searchtype == '2' }">selected="selected"</c:if>>제목</option>
	                            </select>
                            ...
                            <c:choose>
                                	<c:when test="${not empty resultList}">
                                		...
                                		<c:forEach var="result" items="${resultList}" varStatus="status">
		                                    <tr  onclick="view('${result.recsroom_no}');" style="cursor:pointer">
		                                        <td><c:out value="${(rowNumber)-status.count +1}" /></td>
		                                        <td><c:out value="${result.title}" /></td>
		                                        <td><c:out value="${result.writer}" /></td>
		                                        <td>
		                                        	<fmt:parseDate value="${result.wri_date}" var="dateFmt" pattern="yyyyMMdd"/>
    ...
    <!-- js File import -->
	<script type="text/javascript" src="<c:url value='/js/cms/openYard/recsroomList.js' />"></script>

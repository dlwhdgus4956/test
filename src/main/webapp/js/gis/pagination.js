...
function renderPagination() {
	...
	if(currentPage > pageViewCnt) {
		...
		html += "<li><a href='#;' onclick='goList(1)' title='처음 페이지'> << </a></li>";
	}else {
		pageStartNum = 1;
	...
	if(currentPage > 1) {
		html += "<li><a href='#;' onclick='goList("+(parseInt(currentPage)-1)+")' title='이전 페이지'> < </a></li>";
	}
	for(var i=pageStartNum; i <= pageEndNum; i++) {
		...
		html += "><a href='#;' onclick='goList("+i+")' style='font-size:13px;'>"+i+"</a></li>";
	}
	if(totalPage > 1 && currentPage < totalPage) {
		html += "<li><a href='#;' onclick='goList("+(parseInt(currentPage)+1)+")' title='다음 페이지'> > </a></li>";
	}
	if(totalPage > pageViewCnt && currentPage < totalPage) {
		html += "<li><a href='#;' onclick='goList("+totalPage+")' title='마지막 페이지'> >> </a></li>";
	}
